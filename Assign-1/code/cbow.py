...
class DataProcessor:
    ...
    def compute_vocab(self, file):
        ...
        for key, value in sorted_vocab:
            ...
            prob_vocab[key] = (math.sqrt(10000 * float(value)/j) + 1) * \
                (float(1.0)/(10000 * float(value)/j))
    ...
    def make_training_data(self, file, prob_vocab, no_vocab, n=3):
        ...
        for i in range(n+1, len(text)-n):
            if (no_vocab[text[i]] < 15):
                continue
            l += 1
            temp_context = [text[j] for j in range(
                i-n, i+n+1) if (j != i and no_vocab[text[j]] >= 15)]
...
def train(inp, out, dimensions, lr, win, epochs):
    ...
    words_to_int, int_to_words, data_raw = processor.make_training_data(
        inp,  prob_vocab, no_vocab,  win)
    ...
    model = CBoW(words_to_int, int_to_words, X,
                 Y,  lr, dimensions, epochs, True)
    model.build_cbow_model()


parser = argparse.ArgumentParser()
parser.add_argument('--train', help='Training file',
                    dest='inp', default='./data/')
parser.add_argument('-m', '--model', help='Output model file', dest='out')
parser.add_argument('-d', '--dim', help='Dimensionality of word embeddings',
                    dest='dimensions', default=300, type=int)
parser.add_argument('-r', '--rate', help='Learning rate',
                    dest='lr', default=0.025, type=float)
parser.add_argument('-w', '--window', help='Max window length',
                    dest='win', default=3, type=int)
